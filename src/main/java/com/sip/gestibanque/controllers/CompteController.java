...
//	@Autowired
  ...
  public String getFormAddCompte(Model model) {
    ...
    List<Banque> banques = (List<Banque>) this.banqueRepository
        .findAll();
  ...
  public String getAllComptes(Model model) {
    List<CompteBancaire> cbs = (List<CompteBancaire>) compteRepository
        .findAll(); // select *
...
//		CompteBancaire cb = opCompte.get(); // compte qui est rempli depuis la base
  ...
  @PostMapping("/search")
  public String getFormSearchCompte(Model model,
      @RequestParam(value = "proprietaire") String proprietaire) {

    List<CompteBancaire> cbs = compteRepository
        .findByProprietaire(proprietaire);
    model.addAttribute("comptes", cbs);
